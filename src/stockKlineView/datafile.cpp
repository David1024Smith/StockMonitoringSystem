...
void DataFile::calAverageLine()
{
    if (kline.size() > 0) {
        // 初始化各均线的值，但要检查数组边界
        for (int i = 0; i < 4 && i < kline.size(); i++)
            kline[i].averageLine5 = 0;

        for (int i = 0; i < 9 && i < kline.size(); i++)
            kline[i].averageLine10 = 0;

        for (int i = 0; i < 19 && i < kline.size(); i++)
            kline[i].averageLine20 = 0;

        for (int i = 0; i < 29 && i < kline.size(); i++)
            kline[i].averageLine30 = 0;

        for (int i = 0; i < 59 && i < kline.size(); i++)
